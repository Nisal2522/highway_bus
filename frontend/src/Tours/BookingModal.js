...
const BookingModal = ({ isOpen, onClose, selectedPackage, packageData }) => {
  ...
  useEffect(() => {
    ...
    total = total * (Number(formData.numberOfDays) || 1);

    setCalculatedPrice(total);
  }, [selectedPackage, formData.breakfast, formData.hotel, formData.transport, formData.numberOfDays]);
    ...
    <div className="booking-modal-overlay" onClick={handleClose}>
      <div className="booking-modal" onClick={(e) => e.stopPropagation()}>
        <div className="booking-modal-header">
          <h2>Book {options?.packageTitle || 'Package'}</h2>
              ...
              <div className="price-summary">
                <div className="price-breakdown">
                  <div className="price-item">
                    <span>Base Price ({selectedPackage?.charAt(0).toUpperCase() + selectedPackage?.slice(1)}) - {formData.numberOfDays} {formData.numberOfDays === 1 ? 'Day' : 'Days'}</span>
                    <span>Rs. {(options.basePrice * formData.numberOfDays).toLocaleString()}</span>
                  </div>
                  {formData.breakfast && (
                    <div className="price-item">
                      <span>Meal ({formData.breakfast}) - {formData.numberOfDays} {formData.numberOfDays === 1 ? 'Day' : 'Days'}</span>
                      <span>Rs. {((options.breakfast.find(b => b.name === formData.breakfast)?.price || 0) * formData.numberOfDays).toLocaleString()}</span>
                    </div>
                  )}
                  {formData.hotel && (
                    <div className="price-item">
                      <span>Hotel ({formData.hotel}) - {formData.numberOfDays} {formData.numberOfDays === 1 ? 'Day' : 'Days'}</span>
                      <span>Rs. {((options.hotels.find(h => h.name === formData.hotel)?.price || 0) * formData.numberOfDays).toLocaleString()}</span>
                    </div>
                  )}
                  {formData.transport && (
                    <div className="price-item">
                      <span>{selectedPackage === 'premium' ? 'Buses (Private Transport)' : 'Buses'} ({formData.transport}) - {formData.numberOfDays} {formData.numberOfDays === 1 ? 'Day' : 'Days'}</span>
                      <span>Rs. {((options.transport.find(t => t.name === formData.transport)?.price || 0) * formData.numberOfDays).toLocaleString()}</span>
                    </div>
                  )}
                </div>
          ...
          <div className="booking-confirmation">
            ...
            <div className="confirmation-details">
              ...
              <div className="confirmation-section">
                ...
                <p><strong>Duration:</strong> {formData.numberOfDays} {formData.numberOfDays === 1 ? 'Day' : 'Days'}</p>
